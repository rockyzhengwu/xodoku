...
pub struct NackedSetFinder {
    degree: u8,
}

impl NackedSetFinder {
    pub fn new(degree: u8) -> Self {
        NackedSetFinder { degree }
    }
...
impl SolverStrategy for NackedSetFinder {
...
mod test {
    use crate::{
        grid::Grid,
        solver::{
            SolverStrategy, nacked_set::NackedSetFinder, step_accumulator::AllStepAccumulator,
        },
    };
    ...
    fn test_nacked_pair() {
        ...
        let finder = NackedSetFinder::new(2);
    ...
    fn test_nacked_tripe() {
        ...
        let grid = Grid::new_from_hodoku_line(s).unwrap();
        let finder = NackedSetFinder::new(3);
        let mut acc = AllStepAccumulator::default();
        finder.find_step(&grid, &mut acc);
        let steps = acc.get_steps();
    ...
    fn test_nacked_quadruple() {
        ...
        let grid = Grid::new_from_hodoku_line(s).unwrap();
        let finder = NackedSetFinder::new(4);
        let mut acc = AllStepAccumulator::default();
        finder.find_step(&grid, &mut acc);
        let steps = acc.get_steps();
