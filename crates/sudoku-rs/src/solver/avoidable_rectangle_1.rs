...
impl AvoidableRectangleType1 {
    pub fn apply(&self, grid: &mut Grid) {
        assert!(
            ...
            "avoidable rectangle type 1 must remove at least one candidate"
        );
        for cand in self.remove_candidates.iter() {
            assert!(
                grid.remove_candidate(cand),
                "avoidable rectangle type 1 attempted to remove missing candidate {cand:?}"
            );
        }
    }

    pub fn explain(&self) -> String {
        "<h3>Avoidable Rectangle Type 1</h3>".to_string()
    }
}
...
mod test {
    ...
    #[test]
    fn test_avoidable_rectangle_type1_rejects_given_cells() {
        let mut grid = Grid::default();
        assert!(grid.set_value(0, 1, true));
        assert!(grid.set_value(1, 2, false));
        assert!(grid.set_value(27, 2, false));
        let solver = AvoidableRectangleType1Finder::default();
        let mut acc = AllStepAccumulator::default();
        solver.find_step(&grid, &mut acc);
