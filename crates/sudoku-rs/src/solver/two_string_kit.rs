...
impl TwoStringKit {
    pub fn apply(&self, grid: &mut Grid) {
        assert!(
            ...
            "2-string kite must remove at least one candidate"
        );
        for cand in self.remove_candidates.iter() {
            assert!(
                grid.remove_candidate(cand),
                "2-string kite attempted to remove missing candidate {cand:?}"
            );
        }
    }

    pub fn explain(&self) -> String {
        "<h3>2-String Kite</h3>".to_string()
    }
}
        ...
        for value in 1..=9 {
            for r in HouseType::Row.houses().iter() {
                ...
                for c in HouseType::Column.houses().iter() {
                    ...
                    if shared_blocks != 1 || fin_cells.len() != 2 {
                        continue;
                    }
                    fin_cells.sort_unstable();
                    ...
                    let remove_cells: Vec<u8> = see_fin_cells
                        .iter()
                        .filter(|c| {
                            !row_cells.contains(*c)
                                && !col_cells.contains(*c)
                                && grid.cell_has_candidate(*c, value)
                        })
                        .collect();
                    ...
                    if seen_removals.insert(step.remove_candidates.clone())
                        && acc.add_step(Step::TwoStringKit(step))
                    {
                        return;
                    }
                }
...
mod test {
    ...
    fn test_two_string_kit() {
        let _s = ":0401:5:.81.2.+6...+4+2.+6..+89.+568..+24.+6+931+4+27+5+8+4+28+357916+1+7+5+6+8+9+3+245+1..+36+89223...84+6.+8+6.+2.....::524:";
