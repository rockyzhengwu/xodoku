...
impl SueDeCoq {
    pub fn apply(&self, grid: &mut Grid) {
        for cand in self.remove_candidates.iter() {
            assert!(grid.remove_candidate(cand));
        }
    }
...
#[derive(Default)]
pub struct ExtendedSueDeCoqFinder {}
...
impl SueDeCoqFinder {
    ...
    fn find_extended(&self, grid: &Grid, acc: &mut dyn StepAccumulator, house_type: HouseType) {
        for h in house_type.houses() {
            let h_cells = get_house_cell_set(h);
            for b in HouseType::Block.houses() {
                let b_cells = get_house_cell_set(b);
                let intersection = h_cells.intersect(&b_cells).values();
                for common_cells in intersection
                    .into_iter()
                    .filter(|cell| grid.get_value(*cell) == 0)
                    .combinations(2)
                    .chain(
                        h_cells
                            .intersect(&b_cells)
                ...
                {
                    let common_values = values(grid, &common_cells);
                    if common_values.count() < common_cells.len() as u8 + 2 {
                        continue;
                    }
                    let row_cells = h_cells
                        .difference(&b_cells)
                        .iter()
                        .filter(|cell| grid.get_value(*cell) == 0)
                        .collect_vec();
                    let block_cells = b_cells
                        .difference(&h_cells)
                        .iter()
                        .filter(|cell| grid.get_value(*cell) == 0)
                        .collect_vec();
                    for row_size in 1..=3.min(row_cells.len()) {
                        for row_set in row_cells.iter().copied().combinations(row_size) {
                            let row_values = values(grid, &row_set);
                            for block_size in 1..=3.min(block_cells.len()) {
                                for block_set in
                                    block_cells.iter().copied().combinations(block_size)
                                {
                                    let block_values = values(grid, &block_set);
                                    if !row_values
                                        .intersect(&block_values)
                                        .intersect(&common_values)
                                        .is_empty()
                                    {
                                        continue;
                                    }
                                    let extra = row_values
                                        .union(&block_values)
                                        .difference(&common_values)
                                        .count()
                                        as usize;
                                    if row_set.len() + block_set.len()
                                        != common_values.count() as usize - common_cells.len()
                                            + extra
                                    {
                                        continue;
                                    }
                                    let row_remove =
                                        row_values.union(&common_values.difference(&block_values));
                                    let block_remove =
                                        block_values.union(&common_values.difference(&row_values));
                                    let mut remove_candidates = removals(
                                        grid,
                                        h_cells,
                                        &common_cells,
                                        &row_set,
                                    ...
                                    remove_candidates.extend(removals(
                                        grid,
                                        b_cells,
                                        &common_cells,
                                        &block_set,
                                    ...
                                    remove_candidates.sort_by_key(|candidate| {
                                    ...
                                    let step = SueDeCoq {
                                        remove_candidates,
                                        block_candidates: candidates(grid, &block_set),
                                        row_col_candidates: candidates(grid, &row_set),
                                        other_candidates: vec![],
                                        common_candidates: candidates(grid, &common_cells),
                                        block: b,
                                        row_or_cloumn: h,
                                    };
                                    if acc.add_step(Step::SueDeCoq(step)) {
                                        return;
                                    }
                                }
                            }
...
impl SolverStrategy for ExtendedSueDeCoqFinder {
    fn find_step(&self, grid: &Grid, acc: &mut dyn StepAccumulator) {
        SueDeCoqFinder::default().find_extended(grid, acc, HouseType::Row);
        if acc.is_finish() {
            return;
        }
        SueDeCoqFinder::default().find_extended(grid, acc, HouseType::Column);
    }
    fn name(&self) -> &str {
        "ExtendedSueDeCoqFinder"
    }
}

fn values(grid: &Grid, cells: &[u8]) -> DigitSet {
    cells.iter().fold(DigitSet::new_empty(), |values, cell| {
        values.union(&grid.get_cell_candidate(*cell))
    })
}
fn candidates(grid: &Grid, cells: &[u8]) -> Vec<Candidate> {
    cells
        .iter()
        .flat_map(|cell| {
...
fn removals(
    grid: &Grid,
    house: crate::util::indexset::IndexSet,
    common: &[u8],
    selected: &[u8],
    values: DigitSet,
) -> Vec<Candidate> {
    house
        .iter()
        .filter(|cell| !common.contains(cell) && !selected.contains(cell))
        .flat_map(|cell| {
            values
                .iter()
                .filter(move |value| grid.cell_has_candidate(cell, *value))
