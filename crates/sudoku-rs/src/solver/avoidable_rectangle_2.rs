...
impl AvoidableRectangleType2 {
    pub fn apply(&self, grid: &mut Grid) {
        assert!(
            ...
            "avoidable rectangle type 2 must remove at least one candidate"
        );
        for cand in self.remove_candidates.iter() {
            assert!(
                grid.remove_candidate(cand),
                "avoidable rectangle type 2 attempted to remove missing candidate {cand:?}"
            );
        }
    }

    pub fn explain(&self) -> String {
        "<h3>Avoidable Rectangle Type 2</h3>".to_string()
    }
}
    ...
    pub fn find_hint(&self, grid: &Grid, acc: &mut dyn StepAccumulator) {
        ...
        for rows in row_permutations.iter() {
            for cols in col_permutations.iter() {
                ...
                if filled_cells.iter().any(|cell| grid.cell_is_given(*cell)) {
                    continue;
                }
...
mod test {
    ...
    fn test_find_avoidabvle_rectangle_type2() {
        let _s = ":0607:9:+95+4+3..+1.+7+76+15.42..+2+38.71...4+1...36.8.+2....+9.+189.1..7..3...+1+9.+7+2..+92.7+31.+172.3..9.:639:929:";
        let s = r#".---------------.---------------.-----------.
