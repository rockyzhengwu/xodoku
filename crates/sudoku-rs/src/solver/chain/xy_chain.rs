    ...
    pub fn find_xy_chain(&self, grid: &Grid, acc: &mut dyn StepAccumulator) {
        ...
        for (key, edges) in graph.edges.iter() {
            for edge in edges.iter() {
                if edge.end.cell() == key.cell() {
                    ...
                    chain.add_inference(Inference::new(*key, edge.end, InferenceType::Strong));
        ...
        while !queue.is_empty() {
            ...
            let used: HashSet<Candidate> = current_chain
                ...
                .map(|inf| inf.start)
                .collect();
            let used_end: HashSet<Candidate> =
                current_chain.inferences.iter().map(|inf| inf.end).collect();
                ...
                if chain.cells_num() >= 4 {
                        ...
                        && last.inference_type == InferenceType::Strong
                    {
                        ...
                        if !remove_cells.is_empty() {
                            let mut remove_candidates: Vec<Candidate> = remove_cells
                                .iter()
                                .map(|c| Candidate::new(*c, remove_v))
                                .collect();
                            sort_candidates(&mut remove_candidates);
                            let hint = ChainStep {
                                chain: chain.clone(),
                                remove_candidates,
