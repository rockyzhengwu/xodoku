...
impl Graph {
        ...
        for cell in 0..81 {
            ...
            for v in cell_candidate.iter() {
                for h in houses.iter() {
                    let cand_cells = grid.candidate_cells_in_house(*h, v);
    ...
    pub fn new_x_chain_graph(grid: &Grid, x: u8) -> Self {
        ...
        for cell in 0..81 {
            ...
            for h in get_cell_house(cell) {
                let pential_cells = grid.candidate_cells_in_house(h, x);
        ...
        for cell in 0..81_u8 {
            ...
            if candidates.count() == 2 {
                ...
                for start_value in candidates.iter() {
                    for end_value in candidates.iter() {
                        ...
                        graph.add_link(Candidate::new(cell, start_value), end, LinkType::Weak);
                    }
                }
            }
            ...
            for h in cell_houses.iter() {
                for v in candidates.iter() {
                    let mut pential_cells = grid.candidate_cells_in_house(*h, v);
                    ...
                    if pential_cells.count() == 1 {
                        for end in pential_cells.iter() {
                            ...
                            graph.add_link(Candidate::new(cell, v), end_node, LinkType::Strong);
                            used.push(end_node);
                        }
                    } else {
                        for end in pential_cells.iter() {
                            ...
                            graph.add_link(Candidate::new(cell, v), end_node, LinkType::Weak);
                            used.push(end_node);
                        }
                    }
                }
    ...
    pub fn add_link(&mut self, start: Candidate, end: Candidate, link_type: LinkType) {
        ...
        if self.edges.contains_key(&start) && self.edges[&start].contains(&edge_info) {
            return;
        }
...
impl CellGraph {
    ...
    pub fn new_nice_loop_graph(grid: &Grid) -> Self {
        ...
        for cell in 0..81_u8 {
            ...
            for cand in candidates.iter() {
                for h in cell_houses.iter() {
                    let n = grid.get_house_candidate_count(*h, cand);
                    let link_type = if n == 2 {
                        LinkType::Strong
                    } else {
                        LinkType::Weak
                    };
                    for cell2 in grid.candidate_cells_in_house(*h, cand).iter() {
                        if cell2 == cell {
                            continue;
                        }
    ...
    pub fn add_link(&mut self, start: u8, end: Candidate, link_type: LinkType) {
        let edge_info = EdgeInfo { end, link_type };
        if let std::collections::hash_map::Entry::Vacant(e) = self.edges.entry(start) {
            e.insert(vec![edge_info]);
        } else {
