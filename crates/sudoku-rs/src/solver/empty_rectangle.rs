...
impl EmptyRectangle {
    pub fn apply(&self, grid: &mut Grid) {
        assert!(
            ...
            "empty rectangle must remove at least one candidate"
        );
        for cand in self.remove_candidates.iter() {
            assert!(
                grid.remove_candidate(cand),
                "empty rectangle attempted to remove missing candidate {cand:?}"
            );
        }
    }

    pub fn explain(&self) -> String {
        "<h3>Empty Rectangle</h3>".to_string()
    }
}
...
impl EmptyRectangleFinder {
    fn find_er(
        ...
        for b in 18..27 {
            ...
            let block_cells = grid.candidate_cells_in_house(b, value);
            ...
            for pid in 0..9 {
                ...
                let row_cells = grid.candidate_cells_in_house(r, value);
                let col_cells = grid.candidate_cells_in_house(c, value);
                ...
                let pential_col: Vec<u8> = row_cells_out_block
                    .iter()
                    .map(col)
                    .filter(|h| grid.get_house_candidate_count(*h, value) == 2 && *h != c)
                    .collect();
                for pc in pential_col.iter() {
                    let conject_cells = grid.candidate_cells_in_house(*pc, value);
                    for cell in conject_cells.iter() {
                        ...
                        if grid.cell_has_candidate(remove_cell, value) {
                            ...
                            if seen_removals.insert(step.remove_candidates.clone())
                                && acc.add_step(Step::EmptyRectangle(step))
                            {
                                return;
                            }
                        }
                ...
                let pential_row: Vec<u8> = col_cells_out_block
                    .iter()
                    .map(row)
                    .filter(|r| grid.get_house_candidate_count(*r, value) == 2)
                    .collect();
                for pr in pential_row {
                    let conject_cells = grid.candidate_cells_in_house(pr, value);
                    for cell in conject_cells.iter() {
                        let cc = col(cell);
                        if cc == c {
                            continue;
...
impl SolverStrategy for EmptyRectangleFinder {
    fn find_step(&self, grid: &Grid, acc: &mut dyn StepAccumulator) {
        ...
        for value in 1..=9 {
            self.find_er(grid, acc, &mut seen_removals, value);
            if acc.is_finish() {
                return;
            }
        }
